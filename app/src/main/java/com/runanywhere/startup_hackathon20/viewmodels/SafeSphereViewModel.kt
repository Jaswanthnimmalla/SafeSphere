...
class SafeSphereViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val threatMonitor = ThreatMonitor.getInstance(application)
    ...
    // Threat Monitor - Critical Alerts
    val criticalAlertCount = threatMonitor.criticalAlertCount
        .stateIn(viewModelScope, SharingStarted.Eagerly, 0)

    // Combine notification count + critical alert count for bell badge
    val totalAlertCount: StateFlow<Int> = combine(
